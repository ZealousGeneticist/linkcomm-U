...
"""
Copyright 2008,2009,2010,2021 James Bagrow
...
such as tabs might require hitting ctrl+V (bash), consult your shell)", metavar="DELIM")
    ...
    if options.verbose:
        print("Writing integer -> node mapping to %s..." % options.indexFile)
    f = open(options.indexFile, 'w')
    write = f.write
    for index,node in integer2node.items():
        write( "%i%s%s\n" % (index,inDelim,node)  )
    f.close()
    if options.verbose:
        print("done")
